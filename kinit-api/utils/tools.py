...
def get_datetime_timestamp(date_time: str) -> int:
    """
    datetime.datetime 对象转换为毫秒级时间戳
    """
    datetime_obj = datetime.datetime.strptime(date_time, '%Y-%m-%d %H:%M:%S.%f')
    timestamp_ms = (datetime_obj - datetime.datetime(1970, 1, 1)).total_seconds() * 1000
...
def filter_dict_optimized(data_list, keywords: dict[str, int]):
    return [{k: v for k, v in data_dict.items() if k in keywords and v >= keywords[k]} for data_dict in data_list]
